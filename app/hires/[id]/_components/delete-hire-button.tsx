...
export function DeleteHireButton({ hireId }: { hireId: string }) {
  ...
  async function handleDelete() {
    setLoading(true)
    setError(null)
    const res = await fetch(`/api/hires/${hireId}`, { method: 'DELETE' })
    if (!res.ok) {
    ...
    <div>
      {confirming ? (
        <div className="flex items-center gap-3">
          <span className="text-sm text-brand-mid-grey">Delete this hire and all its items?</span>
